(function(window){
    ...
    var Game = {
      ...
      gameOver: function(){
        ...
        var message2 = "monitores mortos: " + Game.score;
        var message3 = "putz você morreu aperta o espaço ou clica tela para recomeçar";
        this.pause();
      ...
      updateScore: function(){
        ...
        this.ctx.fillText("monitores mortos: " + this.score, 8, 20);
    ...
    function displayVictoryMessage() {
        ...
        const message = "Parabéns, Você realmente matou todos eles";
        const message2 = "muito obrigado por está me ajudando, você está com 2 acordos";
        const message3 = "aperte espaço para ir para o proximo desafio";
        Game.ctx.fillText(message, Game.c.width/2 - Game.ctx.measureText(message).width/2, Game.c.height/2);
        Game.ctx.fillText(message2, Game.c.width/2 - Game.ctx.measureText(message).width/1.40, Game.c.height/1.80); 
        Game.ctx.fillText(message3, Game.c.width/2 - Game.ctx.measureText(message).width/2, Game.c.height/1.65); 
        Game.pause();
        // Optionally, pause the game or display additional information
        document.addEventListener('keydown', function(event) {
            if (event.code === 'Space' && Game.ganhou) {
              window.location.href = 'game3/carta.html'; // Substitua por sua URL desejada
            }
          });
        }
        
        // Chamando a função displayVictoryMessage quando o jogador vence
    ...
    Enemy.prototype.die = function(){
      ...
      if(Game.enemiesAlive < Game.maxEnemies){   
        ...
        // Spawns a new enemy after 2 seconds if less than max enemies are alive
        setTimeout(function(){
          new Enemy();
        }, 2000);
      }
     ...
     // Call the function in Enemy.prototype.die
